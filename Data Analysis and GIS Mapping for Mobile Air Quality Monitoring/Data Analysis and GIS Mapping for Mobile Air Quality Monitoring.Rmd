...
The data set processed above was taken on Feb 21, 2024, when the factory was not operating. We also have another data set taken when the factory was operating (on Nov 30,2023). Here I did two sample t-test and Wilcoxon rank sum tests on them. The data set collected on Nov 2023 covers a smaller area in the neighborhood, as that is our first trip and was mainly to test all the devices. To ensure that it's a more equivalent comparison, I retrained a same boundary box for both data sets before comparison. 

### 4.2.1 Prepare two datasets to be compared
```{r}
...
```{r}
# define the fuction for t-test
t_test_2df <- function(df1, df2) {
  ttest_results <- data.frame(mean_df1 = numeric(ncol(df1)-3),
                              mean_df2 = numeric(ncol(df2)-3),
                              p_value = numeric(ncol(df1)-3),
                              row.names = names(df1)[3:(ncol(df1)-1)])
  for (i in 3:(ncol(df1)-1)) {
    ttest <- t.test(df1[[i]], df2[[i]])
    ttest_results[i-2, "mean_df1"] <- round(mean(df1[[i]]),digits=4)
    ttest_results[i-2, "mean_df2"] <- round(mean(df2[[i]]), digits=4)
    ttest_results[i-2, "p_value"] <- round(ttest$p.value, digits=4)
  }
  return(ttest_results)
}

# t test
t.test_result <- t_test_2df(rast_mean_2402_r, rast_mean_2311_r)
t.test_result %>% filter(p_value <= 0.05, mean_df1<mean_df2)
t.test_result %>% filter(p_value <= 0.05, mean_df1>mean_df2)

# From here: need to use a same lamuda for both df
# boxcox transformation
bc_col <- function (df, col_name) {
  df_transformed <- df
  ...
  fit0 <- lm(as.formula(paste(col_name, "~1" )), data = df_transformed, y=TRUE)
  bc_transform <- boxcox(object = fit0,
  ...
  print(is.vector(y_transformed))
}

bc_col(rast_mean_2402_r,"X2.Ethyltoluene")


```
### 4.2.2 Wilcoxon rank sum test
We found three related chemicals show a significant higher concentration when the factory was operating: 2-Ethytolune, Carbon Tetrachloride and trans-1,3-Dichloropropene.
...
To evaluate if there's a linear correlation between the factory's distance and the chemical concentration levels, I conducted a simple linear regression focusing for all the chemicals with higher median concentrations to the northeast of the factory, but non of them looks good. Take Carbon Tetrachloride as an example: the model explain less than 1% of the variation with high P-value; the standardized residuals are not normally distributed and there are a few outliers.
```{r}
...
# higher_in_ne <- c("O.Xylene", "Phenol", "X2.Ethyltoluene", "4","X4.Ethyltoluene", "Ethylbenzene", "Carbon.Tetrachloride", "trans.1.3.Dichloropropene")
# lm_plots <- list()
# for (i in higher_in_ne) {
#   lm.plot <- tryCatch({
#     diag_lm(rast_mean_2311_ne, i)
#   }, error = function(e) {
#     warning(paste("An error occurred with chemical:", i, "Error:", e$message))
#     NULL # to ensure the loop will continue when error occurs
#   })
#   print(lm.plot)
#   lm_plots[[i]] <- lm.plot
# }
...
plot_lm <- function(df, col_name) {
  ...
  adj_r_squared <- round(summary_fit$adj.r.squared, 4)
  intercept <- round(fit$coefficients[1], 3)
  slope <- round(fit$coefficients[2], 3)
  p_value <- round(summary_fit$coefficients["Distance", "Pr(>|t|)"], 4)
  # Creating the plot
